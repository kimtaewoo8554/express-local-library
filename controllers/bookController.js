...
exports.book_delete_post = function(req, res, next) {
    ...
    async.parallel({
      ...
      books_bookinstances: function(callback) {
        BookInstance.find({ 'book' : req.body.bookid}, 'imprint status').exec(callback);
      },
    }, function(err, results) {
        if (err) {return next(err); }
        //Success
        if (results.books_bookinstances.length > 0) {
          ...
          res.render('book_delete', { title: 'Delete_Book', book: results.book, book_bookinstances: results.books_bookinstances});
          return;
        }
        else {
